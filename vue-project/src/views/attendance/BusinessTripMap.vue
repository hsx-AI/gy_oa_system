...
const chinaData = ref({ points: [], lines: [], tree: [], total: 0 })
...
async function initMaps() {
  ...
  const [chinaJson, worldJson] = await Promise.all([
    fetchJson(chinaUrl),
    fetchJson(worldUrl)
  ])
...
async function loadData() {
  ...
  try {
    ...
    chinaData.value = res?.china || { points: [], lines: [], tree: [], total: 0 }
  ...
  return (chinaData.value.lines || [])
    .map(name => {
      ...
      return {
        fromName: '黑龙江省',
        toName: name,
        coords: [HEILONGJIANG_CENTER, coord]
      }
    })
    .filter(Boolean)
})
