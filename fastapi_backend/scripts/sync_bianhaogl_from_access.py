# -*- coding: utf-8 -*-
"""
从旧系统 Access 数据库将 bianhaogl 表全量导入新系统 MySQL。

步骤：
  1. 清空新库 bianhaogl 表
  2. 从 report1.mdb 的 bianhaogl 表读取全部数据（字段与 New 库一致）
  3. 按相同字段全部插入到新库 bianhaogl

使用：
  cd fastapi_backend
  python scripts/sync_bianhaogl_from_access.py [--mdb path/to/report1.mdb] [--dry-run]
...
def read_bianhaogl_from_access(mdb_path: str):
    """
    从 Access 读取 bianhaogl 表全部数据。
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM bianhaogl")
    # 列名：统一小写以兼容 MySQL（与 New 库一致），用反引号包裹避免保留字
...
def main():
    parser = argparse.ArgumentParser(description="从 Access 全量导入 bianhaogl 到 MySQL")
    ...
    print("=" * 60)
    print("bianhaogl 表全量导入（旧 Access -> 新 MySQL）")
    ...
    # 1. 从 Access 读取
    print("\n[1/3] 从 Access 读取 bianhaogl...")
    try:
        col_names, rows = read_bianhaogl_from_access(mdb_path)
    ...
    # 2. 清空新库 bianhaogl
    print("\n[2/3] 清空新库 bianhaogl 表...")
    try:
        db.execute_update("SET FOREIGN_KEY_CHECKS = 0", ())
        n = db.execute_update("TRUNCATE TABLE bianhaogl", ())
        db.execute_update("SET FOREIGN_KEY_CHECKS = 1", ())
    except Exception as e:
        if "TRUNCATE" in str(e):
            n = db.execute_update("DELETE FROM bianhaogl", ())
            print(f"  已用 DELETE 清空（TRUNCATE 不可用时）")
        else:
            raise
    print(f"  已清空")
    ...
    sql = f"INSERT INTO bianhaogl ({', '.join(cols_quoted)}) VALUES ({placeholders})"
    batch_size = 200
    inserted = 0
