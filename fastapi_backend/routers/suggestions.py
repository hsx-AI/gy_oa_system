...
def get_attendance_exception_keys(year: int, month: int, include_buban: bool = False) -> List[tuple]:
    ...
    try:
        ...
        # 排除 yggl 中已标记离职或 lsys 为"其他部门员工/成员"的人员（不参与考勤统计）
        exclude_names = set()
        try:
            ph = ",".join(["%s"] * len(names))
            exc_rows = db.execute_query(
                f"SELECT name FROM yggl WHERE name IN ({ph}) "
                f"AND (COALESCE(zaizhi,0)=1 OR TRIM(lsys) IN ('其他部门员工', '其他部门成员'))",
                tuple(names),
            )
...
def analyze_workday(record: dict, date_obj: datetime) -> List[dict]:
    ...
    # -------------------------------------------------
    # 2. 迟到 / 缺勤检测（基于当天第一次「进」的时间，不拆分跨午休建议）
    #    8 < first < 12   → 迟到：8:00 → first
    #    12 <= first < 13 → 缺勤：8:00 → 12:00（午休内进场，不把午休计入建议）
    #    13 <= first < 17 → 缺勤：8:00 → first（保持原跨午休合并）
    ...
    if WORK_AM_START < first_val < WORK_AM_END:
        ...
        suggestions.append(_sugg(
            "08:00:00", "12:00:00", 1,
            "【考勤建议】检测到缺勤，建议补录 08:00 到 12:00 的考勤"
        ))
    ...
    #    若下次刷入落在午休内，则缺勤只提示到 12:00，避免把午休计入建议。
    # -------------------------------------------------
    for idx, (t_in, t_out) in enumerate(intervals):
        ...
        if idx + 1 < len(intervals):
            ...
            if next_in_val > WORK_PM_END:
                ...
                gap_end = gap_start.replace(hour=WORK_AM_END, minute=0, second=0, microsecond=0)
            else:
                gap_end = next_in
