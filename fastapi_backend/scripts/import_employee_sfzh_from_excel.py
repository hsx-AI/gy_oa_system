# -*- coding: utf-8 -*-
"""
从 Excel 导入身份证号到 yggl.sfzh
- A列：姓名（与 yggl.name 匹配）
- B列：身份证号（写入 sfzh）
按姓名匹配后更新 yggl.sfzh；未在 yggl 中匹配到的姓名写入 未匹配姓名_sfzh.txt。

运行方式（在 fastapi_backend 目录下）:
  python scripts/import_employee_sfzh_from_excel.py [Excel文件路径]
  不传路径时默认使用当前目录下的 身份证号.xls / 身份证号.xlsx
"""
import sys
import os
...
def read_excel_rows(path: str):
    """读取 Excel，返回 [(A列, B列), ...]。A列=姓名，B列=身份证号。"""
...
def main():
    if len(sys.argv) > 1:
        ...
        base = os.getcwd()
        for name in ("身份证号.xls", "身份证号.xlsx"):
            ...
            for name in ("身份证号.xls", "身份证号.xlsx"):
                ...
                excel_path = os.path.join(base, "身份证号.xls")
    ...
    if first_a in ("姓名", "名字", "name") or first_b in ("身份证号", "身份证", "sfzh"):
        rows = rows[1:]
    if not rows:
        print("除表头外无数据")
        return
    ...
    for name, sfzh in pairs:
        ...
        n = db.execute_update("UPDATE yggl SET sfzh = %s WHERE name = %s", (sfzh or "", db_name))
        if n > 0:
            updated += n

    print(f"已按姓名更新 yggl.sfzh：{updated} 条记录。")

    out_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    out_file = os.path.join(out_dir, "未匹配姓名_sfzh.txt")
