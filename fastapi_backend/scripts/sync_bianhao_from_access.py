# -*- coding: utf-8 -*-
"""
从旧系统 Access 数据库将 bianhao（技术编号）表全量导入新系统 MySQL。

步骤：
  1. 清空新库 bianhao 表
  2. 从 report1.mdb 的 bianhao 表读取全部数据（字段与 New 库一致）
  3. 按相同字段全部插入到新库 bianhao

使用：
  cd fastapi_backend
  python scripts/sync_bianhao_from_access.py [--mdb path/to/report1.mdb] [--dry-run]
...
def main():
    parser = argparse.ArgumentParser(description="从 Access 全量导入 bianhao（技术编号）到 MySQL")
    ...
    print("=" * 60)
    print("bianhao 表全量导入（技术编号，旧 Access -> 新 MySQL）")
    ...
    try:
        ...
        db.execute_update("TRUNCATE TABLE bianhao", ())
        db.execute_update("SET FOREIGN_KEY_CHECKS = 1", ())
    except Exception as e:
        if "TRUNCATE" in str(e):
            db.execute_update("DELETE FROM bianhao", ())
    ...
    sql = f"INSERT INTO bianhao ({', '.join(cols_quoted)}) VALUES ({placeholders})"
    batch_size = 200
    inserted = 0
    ...
    print(f"\n完成。共插入 {inserted} 条到 {table_name}（技术编号）。")
